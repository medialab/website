...
module.exports = function oldSlugRedirections(callback) {
  paralell(
    models.map(model => done => {
      ...
      const redirections = [];
      list
        .filter(item => item.oldSlug)
        .forEach(item => {
          redirections.push(
            `rewrite ^/fr/${oldRouteByModel[model]}/${item.oldSlug}/?$ /${newRouteByModel[model]}/${item.slugs[0]} permanent;`
          );
          redirections.push(
            `rewrite ^/${model !== 'people' ? '(en/)?' : ''}${
              oldRouteByModel[model]
            }/${item.oldSlug}/?$ /en/${model}/${item.slugs[0]} permanent;`
          );
        });
    ...
    (err, redirections) => {
      ...
      const pageRedirections = [
        ...
        'rewrite ^/(en/)?projets/?$ /en/activities permanent;',
        ...
        'rewrite ^/(en/)?research-seminar/?$ /en/activities/seminaire-du-medialab permanent;',
        'rewrite ^/fr/contact/?$ /a-propos#contact permanent;',
        'rewrite ^/(en/)?contact/?$ /en/about#contact permanent;',
        'rewrite ^/(fr/|en/)?credits/?$ / permanent;',
        'rewrite ^/(fr/)?people/?$ /equipe permanent;',
        'rewrite ^/(fr/)?publication/?$ /productions permanent;',
        'rewrite ^/en/publication/?$ /en/productions permanent;',
        'rewrite ^/(fr/|en/)?tools/?$ http://tools.medialab.sciences-po.fr permanent;',
        'rewrite ^/(fr/|en/)?projets/reset-modernity/?$ /productions/2016-04-reset-modernity permanent;',
        'rewrite ^/(fr/|en/)?projets/teaching-controversy-mapping/?$ /activites/relation-sciences-et-societes-et-controverses-socio-techniques permanent;',
        ...
        'rewrite ^/(fr/|en/)?blog/culture-numerique/?$ /productions/2019-03-culture-numerique-dominique-cardon permanent;',
