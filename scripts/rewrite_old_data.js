...
const translations = {
  activities: (o, indices, reverseLinks) => {
    ...
    if (o.sstitre_projet_fr) newProject.baseline = {fr: o.sstitre_projet_fr};
    if (o.sstitre_projet_en)
      Object.assign(newProject.baseline, {en: o.sstitre_projet_en});
    if (o.date_debut) newProject.startDate = formatDate(o.date_debut);
    if (o.date_fin) newProject.endDate = formatDate(o.date_fin);
    ...
    if (reverseLinks['activities->people'][newProject.oldSlug])
      newProject.people =
        reverseLinks['activities->people'][newProject.oldSlug];
  ...
  people: (people, indices) => {
    ...
    return {
      newO: newPeople,
      reverseLinks: {activities: people.projets, productions: people.tools}
    };
  },
  news: (o, indices) => {
    const newNews = {
    ...
    if (
      ...
      if (o.date_debut_fr) {
        const groups = o.date_debut_fr.match(
          /\w+ (\d{1,2}) ([\wé]*) ?(\d{4})?/u
        );
        if (groups) {
          const y = !!groups[3]
    ...
    o.people.forEach(p => {
      ...
      if (indices.people[pp]) newNews.people.push(indices.people[pp]);
      else console.log("can't find people", pp);
    });
    newNews.activities = [];
    o.projets.forEach(p => {
      ...
      else console.log("can't find activity", pp);
    });
  ...
  productions: (o, indices, reverseLinks) => {
    ...
    if (reverseLinks['productions->people'][newProduction.oldSlug])
      newProduction.people =
        reverseLinks['productions->people'][newProduction.oldSlug];
  ...
  async.each(
    ...
    (f, localDone) => {
      ...
      if (newO.oldSlug !== 'medialab-dev.sciences-po.fr') {
        // writting
        fs.writeJsonSync(path.join(outPath, `${newO.id}.json`), newO, {
          spaces: 2,
          encoding: 'utf8'
        });
      }
      ...
      async.each(
        ...
        (a, cb) => {
          if (a.newPath !== a.oldPath) {
            if (
              a.oldPath ===
              'onjour%20Bruno,%20%20Voici%20le%20lien%20vers%20l%27affiche%20corrigée%20:%20http://www.medialab.sciences-po.fr/wp-content/uploads/2013/04/affiche.jpg'
            )
              a.oldPath = '/wp-content/uploads/2013/04/affiche.jpg';
            fs.copy(
              path.join('./scripts/wordpress_scraping/data/', a.oldPath),
              path.join(
                './scripts/wordpress_scraping/data/new/assets',
                a.newPath
              ),
              {overwrite: false},
              err => {
                if (err) console.error(a.oldPath, o.permalink);
                cb();
              }
            );
...
async.waterfall([
  ...
  (indices, links, done) => {
    ...
    const hyphe = fs.readJsonSync(
      `./scripts/wordpress_scraping/data/new/productions/${hypheId}.json`,
      'utf8'
    );
    ...
    const aime = fs.readJsonSync(
      `./scripts/wordpress_scraping/data/new/activities/${aimeId}.json`,
      'utf8'
    );
    ...
    fs.writeJSONSync(
      './scripts/wordpress_scraping/data/new/settings.json',
      {settings: {home: {editorialization: []}}},
      {spaces: 2, encoding: 'utf8'}
    );
    done();
  }
