...
function TimeNews(news) {
  ...
  const iconBetween = (
    <span className="between" aria-label="Jusqu'à / to">
      ⇥
    </span>
  );
  ...
  if (endDate) {
    ...
    if (endDateString.length > 10) {
      ...
      timeNews = (
        <p className="hours-news" aria-label="Horaire / Schedule">
          <time dateTime={startTimeFormat}>{startTimeFormat}</time>{' '}
          {iconBetween} <time dateTime={endTimeFormat}>{endTimeFormat}</time>
        </p>
      );
    }
  } else {
