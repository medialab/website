            ...
            <Link to={item.permalink[lang]}>
              {item.model !== 'productions' && (
                <div className="image-pre" aria-hidden="true">
                  <ProcessedImage
                    size="small"
                    image={item.coverImage && item.coverImage.processed.small}
                    data={item}
                  />
                </div>
              )}
                ...
                <>
                  <div className="bandeau">
                    <p className="type-production" data-icon="production">
                      {I18N_TYPE_LABELS.productions[lang][item.type]}
                    </p>
                  ...
                  <hgroup>
                    {item.title && (
                      <h1 data-level-1="title">
                        <LanguageFallback
                          translatedAttribute={item.title}
                          lang={lang}
                        />
                      </h1>
                    )}
              ...
              {item.model === 'people' && (
                <>
                  <div className="bandeau">
                    <p className="type-people" data-icon="people">
                      {i18n[lang].member}
                    </p>
                  </div>
                  <hgroup>
                ...
                <>
                  <div className="bandeau">
                    <p data-icon="news" className="type-news">
                      ...
                      {item.label && (
                        <span>
                          , {item.label[lang] || item.label.fr || item.label.en}
                        </span>
                      )}
                    </p>
                    <DateNews
              ...
              {item.model === 'activities' && (
                <>
                  <div className="bandeau">
                    <p className="type-activity" data-icon="activite">
                      {I18N_TYPE_LABELS.activities[lang][item.type]}
                    </p>
                  ...
                  <hgroup>
                    {item.baseline && (
                      <h1 data-level-1="baseline">
                        <LanguageFallback
                          translatedAttribute={item.baseline}
                          lang={lang}
                        />
                      </h1>
                    )}
