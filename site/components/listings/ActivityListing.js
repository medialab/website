...
export default function ActivityListing({lang, list, topActivities}) {
  ...
  const activities = topActivities
    ...
    .concat(
      list.sort(sortActivities).filter(a => topActivities.indexOf(a.id) === -1)
    );
                ...
                <Link to={a.permalink[lang]}>
                  <div className="image-pre" aria-hidden="true">
                    <ProcessedImage
                      ...
                      image={
                        a.coverImage &&
                        a.coverImage.processed &&
                        a.coverImage.processed.medium
                      }
                      data={a}
                    />
                  </div>
                  <div className="bandeau">
                    <p data-icon="activite" className="type-activity">
                      {
                        a.pinned
                          ? messagesMeta.pinnedActivity[lang]
                          : I18N_TYPE_LABELS.activities[lang][a.type]
                        // I18N_TYPE_LABELS.activities[lang][a.type]
                      }
                    </p>
                    {!a.pinned && (
                      <p className="title" itemProp="name" data-level-2="title">
                        {a.name.fr || a.name.en}
                      </p>
                    )}
                  </div>
                  <hgroup>
                    <h1 data-level-1="baseline">
                      <LanguageFallback
                        lang={lang}
                        translatedAttribute={a.baseline}
                      />
                    </h1>
                  </hgroup>
                  {!a.pinned && (
                    <div className="accroche">
                      <p className="accroche-paragraphe">
                        <LanguageFallback
                          lang={lang}
                          translatedAttribute={a.description}
                        />
                      </p>
                    </div>
                  )}
