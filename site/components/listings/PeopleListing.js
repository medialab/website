...
const i18n = {
  fr: {
    ...
    membership(p) {
      if (p.membership === 'member') return 'Membre';

      if (p.membership === 'associate') return 'Associé·e';
  ...
  en: {
    ...
    membership(p) {
      if (p.membership === 'member') return 'Member';

      if (p.membership === 'associate') return 'Associate';
    ...
    <li
      ...
      <Link to={people.permalink[lang]}>
        ...
        <div className="description">
          <hgroup>
            <h1 data-level-1="name">
              <span itemProp="givenName">{people.firstName}</span>{' '}
              <span itemProp="familyName">{people.lastName}</span>
            </h1>
            <p itemProp="description" className={`status ${lang}`}>
              <LanguageFallback
                lang={lang}
                translatedAttribute={people.status}
              />
            </p>
          </hgroup>
          <div className="details">
            <p className="role">
              {people.role[lang] || people.role.fr || people.role.en}
            </p>
            <p className="details-statut">{i18n[lang].membership(people)}</p>
            <p className="domaine">
              {I18N_TYPE_LABELS.people[lang][people.domain]}
            </p>
          </div>
        </div>
      </Link>
      ...
      <main role="main" aria-describedby="aria-accroche">
        ...
        <section id="liste_equipe" className="main-container">
          <ul className="liste_equipe" id="liste_equipe_active">
            {activePeople.map(p => (
              <PeopleListingItem key={p.permalink.fr} lang={lang} people={p} />
            ))}
          ...
          <input
            type="checkbox"
            id="display-people-past"
            name="display-people-past"
            value="display-people-past"
            hidden
          />
          <label htmlFor="display-people-past">
            {i18n[lang].showPastMembers} <span>〉</span>
          </label>
