...
const ToolFilter = ({lang}) => {
  const {accroche, filtersAlt, infosAlt, closeAlt, filtresTitle} = i18n[lang];
  ...
  return (
    <>
      <h1 className="type_title">
        <a href="#liste">{I18N_MODEL[lang].tools}</a>
      </h1>
      ...
      <aside className="accroche-title-list">
        <h1 className="aside-title" data-icon="production">
          {I18N_MODEL[lang].tools}
        </h1>
      ...
      {Object.keys(I18N_TYPE_LABELS.toolsUsages[lang]).map(usage => {
        return (
          <input
            key={usage}
            type="checkbox"
            id={`filtre-tool_${usage}`}
            name={`filtre-tool_${usage}`}
      ...
      {Object.keys(I18N_TYPE_LABELS.toolsAudience[lang]).map(audience => {
        return (
          <input
            key={audience}
            type="checkbox"
            id={`filtre-tool_${audience}`}
            name={`filtre-tool_${audience}`}
      ...
      {Object.keys(I18N_TYPE_LABELS.toolsStatus[lang]).map(statu => {
        return (
          <input
            key={statu}
            type="checkbox"
            id={`filtre-tool_${statu}`}
            name={`filtre-tool_${statu}`}
            ...
            return (
              <li
                ...
                <input
                  ...
                  type="checkbox"
                  id={`checkbox_filtre_${group.key}`}
                  hidden
                  defaultChecked
                />
                <label
                  className="filter-group-label"
                  htmlFor={`checkbox_filtre_${group.key}`}>
                  <span>
                    <Icons icon="arrow" />
                  </span>
                </label>
                <p className="filter-group-title" aria-hidden="true">
                  {i18n[lang][group.key]}
                </p>
                <div className="filter-group">
                  {Object.keys(I18N_TYPE_LABELS[group.typeKey][lang]).map(
                    usage => {
                      return (
                        <label
                          key={usage}
                          ...
                          htmlFor={`filtre-tool_${usage}`}
                          aria-label={usage}>
                          {I18N_TYPE_LABELS[group.typeKey][lang][usage]}
                        </label>
                      );
                    }
                  )}
